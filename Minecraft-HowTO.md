# How to set up Minecraft
* Install `ATLauncher` (Flatpak)
* Login with Microsoft account
* Install your version of choice
* Add Fabric to Minecraft
* Download `FabricAPI`, `Iris`, `Iris Shaders`, `Sodium`, `FPSDisplay`, `Xaero's Minimap`, `Xaero's World Map`, `Jade`, `Vulkan Mod`, `ComplementaryReimagined` (shaders) (if running on Windows) mods for your version (or check `minecraft-mods` folder and copy it to `mods`) (or restore the backup zip file with ATLauncher `Minecraft1204-2024_04_24_21_21_32.zip`, or `121fabric-2024_07_09_00_22_21.zip`)
* Download `Default-Dark-Mode` (or check `minecraft-resourcepacks` and copy it to `resourcepacks`)
* Change Java version
<img src="./atlauncher-1.png" />
<img src="./atlauncher-2.png" />
* To restore backed up instances - unzip the zip for the version you like - currently 1.20.4 and 1.21 - and copy `mods` folder into the newly installed game
* Import `ComplementaryReimagined_r5.4.zip.txt` if you use the shader
* Play