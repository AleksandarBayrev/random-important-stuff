...
* Install `BleachBit` to clean up unused system files (uncheck `Integrate Shred`).
* Install `GIMP`, `Inkscape`, `Handbrake`, `Discord`, `Kdenlive`, `Avidemux`, `Crystal Disk Info`, `HWINFO`, `HWMonitor`, `OnlyOffice`, `OBS Studio`, `Postman`, `Visual Studio Code`, `qBittorrent`, `VLC`, `WinSCP`, `MediaInfo`, `Audacity`, `FFmpeg Full`, `Vencord`, `Ventoy`, `Image Glass`
...
* Install all extensions to the WSL container as well (open `VS Code Extensions` and you'll see a button `Install to Debian` for example)
* For WSL - run `sudo apt install x11-apps -y` on Ubuntu to be able to run GUI apps through it.
* If you want to install development tools on the local PC - .NET (use `https://github.com/dotnet/install-scripts/blob/main/src/dotnet-install.ps1` and add a PATH variable to `%USERPROFILE%\AppData\Local\Microsoft\dotnet` + add `DOTNET_ROOT` environment variable with value `%USERPROFILE%\AppData\Local\Microsoft\dotnet`), NVM (use `https://github.com/coreybutler/nvm-windows`)
...
* Install necessary drivers (GPU, Chipset (for AMD install them via the AMD install manager))
